...
(eventId, userId, providers, providersOptions, file, title, description, tags) ->
  ...
  publishFileToProviders(userId, providers, providersOptions, file, params).then (results) ->
...
app.get '/*2callback', (req, res) ->
  ...
  if provider is TWITTER
    ...
    getTokens = -> providersAPI.twitter.getAccessToken(oauth_verifier, tokens)
    return
  else
    getTokens = -> providersAPI[provider].pushCode(code, userId)
    return
...
app.get '/events/:userId', (req, res) ->
  userId = req.params.userId
  eventsDAO.retrieveScheduledEventsByUser(userId).then (events) ->
    res.send(events)
  (err) ->
    res.send(err)
...
app.get '/chainedEvents/:eventParentId', (req, res) ->
  ...
  eventsDAO.retrieveChainedEvents(eventParentId).then (events) ->
    res.send(events)
  (err) ->
    res.send(err)
...
app.post '/event/chained/:provider/:eventId/:userId', (req, res) ->
  ...
  scheduledEvent.eventParams .then (result) ->
    res.send(result)
  (err) ->
    res.send(err)
...
app.delete '/token/:provider/:userId', (req, res) ->
  provider = req.params.provider
  userId = req.params.userId
  userDAO.deleteToken(provider, userId).then (result) ->
...
app.post '/publishFromCloud/:userId', (req, res) ->
  ...
  getRefreshedToken(cloudProvider, userId).then (tokens) ->
